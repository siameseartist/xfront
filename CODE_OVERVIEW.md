# Overview of XFront Server Source Code Structure

The XFront Server is a modular codebase that implements a X Window System (X11)
...
Below is a visual overview of how the components of the XFront Server fit
...
- **hw/**: Device Dependant X (DDX) - Hardware-specific code and servers  
    ...
    like `xfree86/` (the main XFront DDX), `kdrive/` (includes Xephyr),
    `vfb/`, `xnest/`, `xquartz/` (macOS), and `xwin/` (Windows). These link
    the various XFront libraries/components (libxserver_*) into executables.
...
- **exa/**: EXA acceleration  
    ...
    for primitives. Primarily used in XFront or Xephyr builds. Depends on
    pixman.
...
- **Xext/**: Core X extensions  
    ...
    for MI support. In XFront, namespace/ adds client isolation/containers.
...
- **Debugging**: Use `-verbose` and `-logverbose` server flags for detailed
  ...
  `your_prefix/lib64/xorg/modules/xfront-25/input` (you can copy this from your
  main XFront `usr/lib/` directory as long as it is ABI-compatible) and place
